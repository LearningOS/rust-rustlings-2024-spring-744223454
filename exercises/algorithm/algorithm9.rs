    ...
    fn next(&mut self) -> Option<T> {
        if self.count == 0 {
            ...
            while idx * 2 <= self.count {
                ...
                if left_idx <= self.count && (self.comparator)(&self.items[left_idx], &self.items[target_idx]) {
                    target_idx = left_idx;
                }

                if right_idx <= self.count && (self.comparator)(&self.items[right_idx], &self.items[target_idx]) {
